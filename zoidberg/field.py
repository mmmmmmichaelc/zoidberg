...
class MagneticField(object):
    ...
    def field_direction(self, pos, ycoord, flatten=False):
        ...
        if np.amin(np.abs(By)) < 1e-12: # TODO michael changed from 1e-8
            # Very small By
            raise ValueError(
...
class MGRID(MagneticField):
    """
    A wrapper for STELLOPT mgrid files

    takes in an mgrid for both the vacuum field
    and the field generated by plasma currents

    currently only works with single groups of coils
    (if mgrid file contains variables such as br_00X,
    where X != 1, this class will not function as
    expected)
    """

    def __init__(self, vacuum_mgrid, plasma_mgrid):

        from scipy.interpolate import RegularGridInterpolator

        vmgrid = Dataset(vacuum_mgrid, "r")
        pmgrid = Dataset(plasma_mgrid, "r")

        vnum_phi = vmgrid.variables["kp"][0]
        vnum_zee = vmgrid.variables["jz"][0]
        vnum_rad = vmgrid.variables["ir"][0]
        vrmin = vmgrid.variables["rmin"][0]
        vrmax = vmgrid.variables["rmax"][0]
        vzmin = vmgrid.variables["zmin"][0]
        vzmax = vmgrid.variables["zmax"][0]

        pnum_phi = vnum_phi
        pnum_zee = vnum_zee
        pnum_rad = vnum_rad
        prmin = pmgrid.variables["rmin"][0]
        prmax = pmgrid.variables["rmax"][0]
        ...
        vrad = np.linspace(vrmin, vrmax, vnum_rad)
        ...
        vbz = vmgrid.variables["bz_001"][...]

        pbp = pmgrid.variables["bp_grid"][...]
        pbr = pmgrid.variables["br_grid"][...]
        pbz = pmgrid.variables["bz_grid"][...]

        vpoints = (vphi, vzee, vrad)
        ppoints = (pphi, pzee, prad)

        self.vbp_rgi = RegularGridInterpolator(vpoints, vbp, bounds_error=False)
        self.vbz_rgi = RegularGridInterpolator(vpoints, vbz, bounds_error=False)
        self.vbr_rgi = RegularGridInterpolator(vpoints, vbr, bounds_error=False)

        self.pbp_rgi = RegularGridInterpolator(ppoints, pbp, bounds_error=False)
        self.pbz_rgi = RegularGridInterpolator(ppoints, pbz, bounds_error=False)
        self.pbr_rgi = RegularGridInterpolator(ppoints, pbr, bounds_error=False)

    def bp_rgi(self, points):
        return self.vbp_rgi(points) + self.pbp_rgi(points)
    
    def bz_rgi(self, points):
        return self.vbz_rgi(points) + self.pbz_rgi(points)
    
    def br_rgi(self, points):
        return self.vbr_rgi(points) + self.pbr_rgi(points)

    def Rfunc(self, x, z, phi):
        return x

    def Bxfunc(self, x, z, phi):
        phi = np.mod(phi, self.max_phi)
        points = (phi, z, x)
        br = self.br_rgi(points)
        # bp = self.bp_rgi(points)
        # return br*np.cos(phi) - bp*np.sin(phi)
        # return (br*np.cos(phi) - bp*np.sin(phi))*np.cos(phi) + (br*np.sin(phi) + bp*np.cos(phi))*np.sin(phi)
        return br
    ...
    def Byfunc(self, x, z, phi):
        ...
        # br = self.br_rgi(points)
        bp = self.bp_rgi(points)
        # return br*np.sin(phi) + bp*np.cos(phi)
        # return -(br*np.cos(phi) - bp*np.sin(phi))*np.sin(phi) + (br*np.sin(phi) + bp*np.cos(phi))*np.cos(phi)
        return bp

    def Bzfunc(self, x, z, phi):
        phi = np.mod(phi, self.max_phi)
        points = (phi, z, x)
        return self.bz_rgi(points)
