...
class AnimateVectorField(object):
    ...
    def __init__(self, X, Y, U, V):
        ...
        self.quiv_plot = self.ax.quiver(
            X, Y, U[0, ...].T, V[0, ...].T, pivot="mid", angles="xy"
        )
    ...
    def animate(self):
        self.animation = anim.FuncAnimation(
            self.fig, self.__update_quiver, frames=self.frames, interval=100, blit=False
        )
        plt.show()
        plt.draw()
...
def plot_forward_map(grid, maps, yslice=0):
    ...
    plt.plot(
        ...
        np.ravel(pol_next.Z),
        "bx",
        label="Grid points on slice {0}".format(yslice + 1),
    )
    ...
    plt.plot(
        ...
        np.ravel(forward_Z),
        "ro",
        label="Forward map from slice {0}".format(yslice),
    )
    ...
    plt.plot(
        np.ravel(forward_R[in_boundary]),
        np.ravel(forward_Z[in_boundary]),
        "ko",
        label="Inner boundary points",
    )

    # Outer boundary marked with x index nx
    out_boundary = maps["forward_xt_prime"][:, yslice, :] > nx - 0.5
    plt.plot(
        np.ravel(forward_R[out_boundary]),
        np.ravel(forward_Z[out_boundary]),
        "bo",
...
def plot_backward_map(grid, maps, yslice=0):
    ...
    plt.plot(
        ...
        np.ravel(backward_Z),
        "ro",
        label="Backward map from slice {0}".format(yslice),
    )
    ...
    plt.plot(
        np.ravel(backward_R[in_boundary]),
        np.ravel(backward_Z[in_boundary]),
        "ko",
        label="Inner boundary points",
    )
    ...
    plt.plot(
        np.ravel(backward_R[out_boundary]),
        np.ravel(backward_Z[out_boundary]),
        "bo",
        label="Outer boundary points",
    )
